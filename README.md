...
- `tests/notes/note-management.spec.ts` - Favorite/Archive/Move to trash/un-Favorite/un-Pin (all reached via the list item's right-click context menu - there's no hover/ellipsis button) and editing an existing note's title
...
- `tests/notes/organization.spec.ts` - linking a note to a newly created notebook, browsing it via the top rail's Notebooks icon, and creating/assigning a color (colors become their own sidebar collection)
- `tests/notes/insert-blocks.spec.ts` - the "+" Insert menu's Code block and a normal-sized Table both work; Task list does nothing and the smallest ("1 x 1") Table size crashes the app - both documented as bugs via `test.fail()` (see Known issues)
- `tests/notes/sharing.spec.ts` - Copy link puts a `nn://note/<id>` deep link on the clipboard; the full Lock cycle - create vault, lock a note, unlock it with the vault password, confirm the content reads back correctly. Also documents a real edge-case bug: locking an empty (no body) note never shows the locked placeholder (`test.fail()`)
...
**RESOLVED 2026-09-17: Lock/unlock is fully automated now, and the earlier "Open note does nothing" finding (2026-09-15/16) was never a product bug - it was this suite's own mistake.** Verified first in a real (non-headless) browser: the locked view has a genuine password input, "Enter your access code." - easy to miss visually since it renders identically whether empty or not, with no distinguishing placeholder styling in a quick screenshot scan. Every earlier automated attempt clicked "Open note" with that field still empty, which is indistinguishable from a dead button when you're only checking body text/DOM for a *result*, not looking at the field itself. Reproduced the full cycle in headless Playwright immediately after finding this: `NoteContextMenu.lock(noteItem, vaultPassword)` creates the vault and locks the note, `NoteEditorPage.unlockNote(vaultPassword)` fills that field and clicks Open note, `NoteEditorPage.isLocked()` checks for the locked placeholder text. See `tests/notes/sharing.spec.ts`. Lesson: a click that "does nothing" is worth double-checking for a field you didn't notice before concluding the button itself is broken - the general headless-vs-real-browser caution was the right instinct, but the specific cause here was simpler than a rendering discrepancy.

**The locked placeholder can take several seconds to render for the tab that was already open during lock creation** - confirmed flaky under headless load (sometimes near-instant, sometimes 3+ seconds with no visible intermediate state). `NoteContextMenu.lock()` waits for the vault-creation dialog to actually close before returning, and callers should `expect.poll()` on `isLocked()` rather than checking once immediately after.

**BUG: locking a note with no body content never shows the locked placeholder - even after closing and reopening it - while the sidebar list item correctly shows a lock icon.** Confirmed 2026-09-17: the note is genuinely marked locked at the data level (the list icon proves it), but the editor pane just keeps showing the normal empty "Start writing your note..." view instead of enforcing the lock in the UI. Only reproduces on notes with zero body content - a locked note with any text correctly shows the password prompt every time (see the passing "can be unlocked" test). Tracked via `test.fail()` in `sharing.spec.ts`.
...
**`InsertBlockMenu.plusButton` was completely broken and no test had ever caught it.** Its locator was `button:near(:text("Bold"))` - "Bold" is an icon-only button whose label lives in a `title` attribute, not visible text, so `:text()` matched nothing and the whole locator resolved to 0 elements. Nothing in the suite called `insertMenu.open()` before 2026-09-17, so this shipped silently. The "+" button itself carries no title/aria-label/data-test-id at all; the fix locates it structurally as the first `<button>` two DOM levels up from the (title-addressable) Bold button. Worth the same lesson as the `__dirname` issue: code that type-checks and "looks done" can still never have actually run.

**BUG: the Insert menu's "Task list" item does nothing.** Confirmed 2026-09-17: clicking `menu-button-tasklist` closes the menu but leaves the body as a single plain `<p>` - no `<ul class="simple-checklist">` gets created, even after typing. This is a different code path from the FormattingToolbar's own More -> Checklist (see `formatting.spec.ts`), which works correctly and produces exactly that structure - so the underlying checklist feature isn't broken, just this specific menu entry point to it. The keyboard shortcut shown next to the item (⌘⇧T) isn't a usable cross-check either - it's intercepted by Chrome itself (reopen-closed-tab) before reaching the page. Tracked via `test.fail()` in `insert-blocks.spec.ts`.

**Table, in the Insert menu, opens its own nested submenu with a hover-to-preview size-picker grid** (`[data-index="N"]` cells, no per-cell `data-test-id`; hovering one updates a "R x C" text label; there's a separate "Import CSV" option too, not covered) - confirmed 2026-09-17. A normal size (confirmed from grid index 14, "5 x 2") inserts a working table. **The grid cell must actually be hovered before clicking it, not just clicked directly** - a direct click with no prior hover on that same cell reproduced the crash bug below even at a normal-size index; `InsertBlockMenu.insertTable()` hovers then clicks for this reason.

**BUG: inserting the smallest ("1 x 1") table crashes the whole app.** Confirmed 2026-09-17: selecting grid index 0 throws an uncaught ProseMirror `RangeError: Invalid content for node table: <>`, and the app falls back to the same full-screen "Something went wrong" crash screen as the malformed-JPEG-paste bug (`upload-attachment.spec.ts`) - recoverable only via "Reload app". Any larger size works fine (see the passing table test), so this is isolated to the minimum-size edge case specifically. Tracked via `test.fail()` in `insert-blocks.spec.ts`.
